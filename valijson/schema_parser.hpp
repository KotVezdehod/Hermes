    ...
    public:
            ...
            typename FunctionPtrs<AdapterType>::FreeDoc freeDoc = nullptr)
        {
            if ((fetchDoc == nullptr) ^ (freeDoc == nullptr)) {
                ...
                throwRuntimeError("Работа с удаленным документом не может быть реализована без функций 'fetch' и 'free'");
            }
    ...
    private:
        ...
        bool extractJsonReference(const AdapterType& node, std::string& result)
        {
            ...
            if (itr == o.end()) {
                ...
                throwRuntimeError("$ref поле должно быть строкой.");
            }
            ...
            const Subschema* schema)
        {
            for (const std::string& keyToCreate : keysToCreate) {
                ...
                if (!schemaCache.insert(value).second) {
                    ...
                    throwLogicError("Ключ '" + keyToCreate + "' уже в кэше схемы (это дубль).");
                }
            }
        }
            ...
            if (actualDocumentUri && (!currentScope || *actualDocumentUri != *currentScope)) {
                ...
                if (docCacheItr == docCache.end()) {
                    ...
                    if (!fetchDoc) {
                        ...
                        throwRuntimeError("Извлечение удаленных ссылок JSON не включено.");
                    }
                    ...
                    if (!newDoc) {
                        ...
                        throwRuntimeError("Не удалось получить документ, на который ссылается схема: " + *actualDocumentUri);
                    }
            ...
            SchemaCache& schemaCache)
        {
            static_assert((std::is_convertible<AdapterType,
                ...
                "SchemaParser::populateSchema должен вызываться с соответствующей реализацией адаптера");
            ...
            if (!node.isObject()) {
                if (m_version == kDraft7 && node.maybeBool()) {
                    ...
                    s += "Ожидается, что узел ";
                    s += nodePath;
                    if (m_version == kDraft7) {
                        s += " будет содержать объект схемы или логическое значение; фактический тип: ";
                    }
                    else {
                        s += " будет содержать объект схемы; фактический тип: ";
                    }
            ...
            if ((itr = object.find("divisibleBy")) != object.end()) {
                if (m_version == kDraft3) {
                    if (itr->second.maybeInteger()) {
                        ...
                        throwRuntimeError("В условии"
                            " 'divisibleBy' ожидается числовое значение.");
                    }
                }
                else {
                    throwRuntimeError(
                        //"'divisibleBy' constraint not valid after draft 3");
                        "Такое 'divisibleBy' условие не считается правильным, начиная с релиза спецификации схем 'Draft 3'");
                }
            }
            ...
            if (m_version == kDraft7) {
                ...
                throwRuntimeError("Для условия 'exclusiveMaximum' не установлено значение 'maximum' ");
            }
            ...
            if (m_version == kDraft7) {
                ...
                throwRuntimeError("Для условия 'exclusiveMinimum' не установлено значение 'minimum' ");
            }
            ...
            if ((itr = object.find("multipleOf")) != object.end()) {
                if (m_version == kDraft3) {
                    ...
                    throwRuntimeError("Такое 'multipleOf' условие не считается правильным, начиная с релиза спецификации схем 'Draft 3'");
                    ...
                    throwRuntimeError("Значение условия 'divisibleBy' должно быть числом.");
                }
            }
            ...
            if ((itr = object.find("required")) != object.end()) {
                if (m_version == kDraft3) {
                    if (parentSubschema && ownName) {
                        ...
                        throwRuntimeError("Ошибка в условии 'required'.");
                    }
                }
                else {
            ...
            if ((itr = object.find("title")) != object.end()) {
                if (itr->second.maybeString()) {
                    ...
                    throwRuntimeError("Значение условия 'title' должно быть строкой.");
                }
            }
            ...
            SchemaCache& schemaCache)
        {
            ...
            if (documentUri && (internal::uri::isUriAbsolute(*documentUri) || internal::uri::isUrn(*documentUri))) {
                // Resolve reference against remote document
                if (!fetchDoc) {
                    throwRuntimeError("Получение удаленных ссылок JSON не включено.");
                }
                ...
                // Can't proceed without the remote document
                if (!newDoc) {
                    throwRuntimeError("Не удалось получить ссылочный документ " + *documentUri + " схемы");
                }
            ...
            SchemaCache& schemaCache)
        {
            if (!node.maybeArray()) {
                ...
                throwRuntimeError("Значение условия типа 'allOf' должно быть коллекцией.");
            }
            ...
            for (const AdapterType schemaNode : node.asArray()) {
                if (schemaNode.maybeObject() || (m_version == kDraft7 && schemaNode.isBool())) {
                    ...
                    throwRuntimeError("Все значения условия типа 'allOf' должны быть объектами.");
                }
            }
            ...
            SchemaCache& schemaCache)
        {
            if (!node.maybeArray()) {
                ...
                throwRuntimeError("Значение условия типа 'anyOf' должно быть коллекцией.");
            }
            ...
            for (const AdapterType schemaNode : node.asArray()) {
                if (schemaNode.maybeObject() || (m_version == kDraft7 && schemaNode.isBool())) {
                    ...
                    throwRuntimeError("Все значения условия типа 'anyOf' должны быть объектами.");
                }
            }
            ...
            SchemaCache& schemaCache)
        {
            ...
            if (contains.isObject() || (m_version == kDraft7 && contains.maybeBool())) {
                ...
                throwRuntimeError("Условие 'contains' должно быть схемой.");
            }
            ...
            SchemaCache& schemaCache)
        {
            if (!node.maybeObject()) {
                ...
                throwRuntimeError("Условие 'dependencies' должно быть подчиненной схемой.");
            }
                ...
                if (member.second.maybeArray()) {
                    ...
                    for (const AdapterType dependencyName : member.second.asArray()) {
                        if (dependencyName.maybeString()) {
                            ...
                            throwRuntimeError("В условии " + member.first + " типа 'dependencies' все значения списка должны быть строками");
                        }
                    }
                    ...
                    throwRuntimeError("Ошибочное определение зависимостей.");
                }
            }
            ...
            const AdapterType& node)
        {
            ...
            throwRuntimeError("Значение условия 'format' должно быть строкой.");
        }
            ...
            SchemaCache& schemaCache)
        {
            ...
            if (additionalItems) {
                if (additionalItems->maybeBool()) {
                    ...
                    throwRuntimeError("Значение условия 'additionalItems' должно быть объектом или bool");
                }
            }
            else {
            ...
            if (items) {
                if (items->isArray()) {
                    ...
                    throwRuntimeError("Значение не единичного условия 'items' должно быть коллекцией.");
                }
            }
            ...
            SchemaCache& schemaCache)
        {
            ...
            if (items.isObject() || (m_version == kDraft7 && items.maybeBool())) {
                ...
                throwRuntimeError("Значение для единичного условия 'items' должно быть схемой.");
            }
            ...
            const AdapterType* exclusiveMaximum)
        {
            if (!node.maybeDouble()) {
                ...
                throwRuntimeError("Для условия 'maximum' значение должно быть числом.");
            }
            ...
            if (exclusiveMaximum) {
                if (!exclusiveMaximum->maybeBool()) {
                    ...
                    throwRuntimeError("Для условия 'exclusiveMaximum' значение должно быть bool.");
                }
        ...
        constraints::MaximumConstraint makeMaximumConstraintExclusive(const AdapterType& node)
        {
            if (!node.maybeDouble()) {
                ...
                throwRuntimeError("Для условия 'exclusiveMaximum' значение должно быть числом.");
            }
            ...
            const AdapterType& node)
        {
            ...
            throwRuntimeError("Для условия 'exclusiveMaximum' значение должно быть не отрицательным числом.");
        }
            ...
            const AdapterType& node)
        {
            ...
            throwRuntimeError("Для условия 'maxLength' значение должно быть не отрицательным числом.");
        }
            ...
            const AdapterType& node)
        {
            ...
            throwRuntimeError("Для условия 'maxProperties' значение должно быть неотрицательным числом.");
        }
            ...
            const AdapterType* exclusiveMinimum)
        {
            if (!node.maybeDouble()) {
                ...
                throwRuntimeError("Для условия 'minimum' значение должно быть числом.");
            }
            ...
            if (exclusiveMinimum) {
                if (!exclusiveMinimum->maybeBool()) {
                    ...
                    throwRuntimeError("Для условия 'exclusiveMinimum' значение должно быть bool.");
                }
        ...
        constraints::MinimumConstraint makeMinimumConstraintExclusive(const AdapterType& node)
        {
            if (!node.maybeDouble()) {
                ...
                throwRuntimeError("Для условия 'exclusiveMinimum' значение должно быть числом.");
            }
        ...
        constraints::MinItemsConstraint makeMinItemsConstraint(const AdapterType& node)
        {
            ...
            throwRuntimeError("Для условия 'minItems' значение должно быть неотрицательным числом.");
        }
        ...
        constraints::MinLengthConstraint makeMinLengthConstraint(const AdapterType& node)
        {
            ...
            throwRuntimeError("Для условия 'minLength' значение должно быть неотрицательным числом.");
        }
        ...
        constraints::MinPropertiesConstraint makeMinPropertiesConstraint(const AdapterType& node)
        {
            ...
            throwRuntimeError("Для условия 'minProperties' значение должно быть неотрицательным числом.");
        }
            ...
            SchemaCache& schemaCache)
        {
            ...
            throwRuntimeError("Для условия типа 'not' значение должно быть объектом.");
        }
            ...
            SchemaCache& schemaCache)
        {
            ...
            if (additionalProperties) {
                ...
                if (additionalProperties->isBool() ||
                    ...
                    throwRuntimeError("Неверный тип значения условия 'additionalProperties'.");
                }
            }
            else {
                ...
                const std::string& name)
        {
            if (!node.maybeBool()) {
                ...
                throwRuntimeError("Значение аттрибута 'required' должно быть bool.");
            }
            ...
            const AdapterType& node)
        {
            ...
            for (const AdapterType v : node.getArray()) {
                if (!v.maybeString()) {
                    ...
                    throwRuntimeError("Наименование 'required' свойства должно быть строкой");
                }
            ...
            SchemaCache& schemaCache)
        {
            ...
            if (node.maybeString()) {
                ...
                if (type == TypeConstraint::kAny && m_version == kDraft4) {
                    ...
                    throwRuntimeError("Тип 'any' не поддерживается начинай с 4й версии спецификации схем.");
                }
                ...
                for (const AdapterType v : node.getArray()) {
                    if (v.maybeString()) {
                        ...
                        throwRuntimeError("Наименование типа должно быть строкой.");
                    }
        ...
        opt::optional<constraints::UniqueItemsConstraint> makeUniqueItemsConstraint(const AdapterType& node)
        {
            ...
            throwRuntimeError("Значение условия 'uniqueItems' должно быть boolean.");
        }
