...
public:
    ...
    bool visit(const AnyOfConstraint &constraint) override
    {
        ...
        if (numValidated == 0 && m_results) {
            ...
            m_results->pushError(m_context, "Не удалось проверить json ни по одной из дочерних схем (связаны условием 'одна из').");
    ...
    bool visit(const ConditionalConstraint &constraint) override
    {
        ...
        if (!validated && m_results) {
            ...
            m_results->pushError(m_context, "Не удалось проверить json ни по одной из двух дочерних схем (связаны условием 'если не, тогда').");
    ...
    bool visit(const ConstConstraint &constraint) override
    {
        if (!constraint.getValue()->equalTo(m_target, m_strictTypes)) {
            if (m_results) {
                ...
                m_results->pushError(m_context, "Значение поля не соответствует константному значению, объявленному в схеме.");
            }
            return false;
        }
    ...
    bool visit(const ContainsConstraint &constraint) override
    {
        ...
        if (!validated) {
            if (m_results) {
                ...
                m_results->pushError(m_context, "Константное значение, установленное в схеме не соответствует ни одному из объявленных в схеме значений.");
            }
    ...
    bool visit(const EnumConstraint &constraint) override
    {
        ...
        if (numValidated == 0) {
            if (m_results) {
                ...
                m_results->pushError(m_context, "Значение не соответствует ни одному из возможных значений перечисления, объявленного в схеме.");
            }
            ...
            if (std::regex_match(s, matches, date_regex)) {
                ...
                if (m_results) {
                    m_results->pushError(m_context,
                                         ...
                                         "Строка должна быть формата 'дата'");
                }
                return false;
            }
        } else if (format == "time") {
            ...
            if (std::regex_match(s, time_regex)) {
                ...
                if (m_results) {
                    m_results->pushError(m_context,
                                         ...
                                         "Строка должна быть формата 'время'");
                }
                return false;
            }
            ...
            if (std::regex_match(s, matches, datetime_regex)) {
                ...
                if (m_results) {
                    m_results->pushError(m_context,
                                         ...
                                         "Строка должна быть формата 'дата-время'");
                }
                return false;
            }
        }
        ...
        if (itemSubschemaCount > 0) {
            if (!additionalItemsSubschema) {
                if (arrSize > itemSubschemaCount) {
                    ...
                    m_results->pushError(m_context, "Коллекция содержит больше значений, чем допускается, согласно схемы.");
                    validated = false;
                }
            }
...
//                                    " against additional items schema.");
                            ...
                            m_results->pushError(m_context, "Элемент коллекции #" + std::to_string(index) +
                                    " не прошел проверку по 'additionalItems' элементам массива.");
                            validated = false;
                        } else {
                            return false;
                        }
...
//                        " or greater using 'items' constraint or 'additionalItems' constraint.");
                ...
                m_results->pushError(m_context, "Элементы коллекции, начиная с #" + std::to_string(numValidated) +
                        " не прошли проверку по 'items' или 'additionalItems' элементам массива.");
                validated = false;
        ...
        if (constraint.getExclusiveMaximum()) {
            if (m_target.asDouble() >= maximum) {
                if (m_results) {
                    ...
                    m_results->pushError(m_context, "Ожидается число меньшее, чем " + std::to_string(maximum));
                }
            ...
            if (m_results) {
                ...
                m_results->pushError(m_context, "Ожидается число меньшее или равное " + std::to_string(maximum));
            }
...
//                    " elements.");
            m_results->pushError(m_context, "Массив должен содержать больше чем " + std::to_string(maxItems) +
                    " элементов.");
...
//                    " characters in length.");
            m_results->pushError(m_context, "Строка должна содержать более чем " + std::to_string(maxLength) +
                    " символов.");
        }
...
//                    " properties.");
            m_results->pushError(m_context, "Объект не должен содержать более чем " + std::to_string(maxProperties) +
                    " полей.");
        }
        ...
        if (constraint.getExclusiveMinimum()) {
            if (m_target.asDouble() <= minimum) {
                if (m_results) {
                    ...
                    m_results->pushError(m_context, "Число должно быть больше, чем " + std::to_string(minimum));
                }
            ...
            if (m_results) {
                ...
                m_results->pushError(m_context, "Число должно быть больше или равно " + std::to_string(minimum));
            }
...
//                    " elements.");
            m_results->pushError(m_context, "Коллекция должна содержать не менее, чем " + std::to_string(minItems) +
                    " элементов.");
        }
...
//                    " characters in length.");
            ...
            m_results->pushError(m_context, "Строка должна быть не короче " + std::to_string(minLength) +
                    " символов.");
        }
...
//                    " properties.");
            m_results->pushError(m_context, "Объект должен содержать не менее " + std::to_string(minProperties) +
                    " полей.");
...
//                            "to a number to check if it is a multiple of " + std::to_string(divisor));
                    m_results->pushError(m_context, "Не удалось преобразовать значение в число "
                            "для проверки его кратности числу " + std::to_string(divisor));
                }
                return false;
            }
...
//                            "to a number to check if it is a multiple of " + std::to_string(divisor));
        ...
        if (fabs(r) > std::numeric_limits<double>::epsilon()) {
            if (m_results) {
                ...
                m_results->pushError(m_context, "Число должно быть кратно " + std::to_string(divisor));
            }
            return false;
        }
    ...
    bool visit(const MultipleOfIntConstraint &constraint) override
    {
        ...
        if (m_target.maybeInteger()) {
            if (!m_target.asInteger(i)) {
                if (m_results) {
                    ...
                    m_results->pushError(m_context, "Не удалось преобразовать значение в число для проверки кратности 'любому из' чисел, входящих в заданный в схеме список.");
                }
                return false;
            }
        ...
        if (i % divisor != 0) {
            if (m_results) {
                //m_results->pushError(m_context, "Value should be a multiple of " + std::to_string(divisor));
                m_results->pushError(m_context, "Число должно быть кратно любому из числел, входящих в заданный в схеме список: " + std::to_string(divisor));
            }
            return false;
        }
            ...
            if (m_results) {
                m_results->pushError(m_context,
                        //"Target should not validate against schema specified in 'not' constraint.");
                          "Провалена валидация цели по принципу 'не должна соответствовать схеме'.");
            }
...
//                        "child schemas allowed by oneOf constraint.");
                ...
                m_results->pushError(m_context, "Провалена валидация по дочерним схемам, согласно принципу 'соответсвует ХОТЯ-БЫ одной из'");
            }
            return false;
        } else if (numValidated != 1) {
            if (m_results) {
                //m_results->pushError(m_context, "Failed to validate against exactly one child schema.");
                m_results->pushError(m_context, "Провалена валидация по дочерним схемам, согласно принципу 'соответсвует ТОЛЬКО одной из'.");
            }
            return false;
        }
        ...
        if (!additionalPropertiesSubschema) {
            if (propertiesMatched.size() != m_target.getObjectSize()) {
                if (m_results) {
                    ...
                    m_results->pushError(m_context, "Объект содержит поле '"  + unwanted + "', "
                            "которое не соответствует описанию коллекции полей ('properties') "
                            "или дополнительных полей ('additionalProperties') схемы.");
                }
            ...
            if (propertiesMatched.find(m.first) == propertiesMatched.end()) {
                ...
                if (!validator.validateSchema(*additionalPropertiesSubschema)) {
                    if (m_results) {
                        ...
                        m_results->pushError(m_context, "Провалена валидация значения по дополнительным полям ('additionalProperties') схемы");
                    }
        ...
        for (const AdapterType &item : m_target.getArray()) {
            ...
            if (!validationVisitor.validateSchema(*itemsSubschema)) {
                if (m_results) {
					...
					m_results->pushError(m_context, "Провал валидации элемента коллекции #" + std::to_string(index));
                    validated = false;
                } else {
                    return false;
                }
    ...
    bool visit(const TypeConstraint &constraint) override
    {
        ...
        // Check schema-based types
        {
            ...
            if (numValidated > 0) {
				...
				m_results->pushError(m_context, "Тип значения поля не разрешен.");
            }
        }
...
//                        + " and #" + std::to_string(innerIndex) + " violate uniqueness constraint.");
                    ...
                    m_results->pushError(m_context, "Элементы с индексами#" + std::to_string(outerIndex)
                        + " и #" + std::to_string(innerIndex) + " нарушают принцип уникальности, объявленный в схеме.");
    ...
    struct ValidateEquality
    {
        ...
        bool operator()(const OtherValue &value) const
        {
            ...
            if (m_results) {
                ...
                m_results->pushError(m_context, "Целевое и сравнивниваемое значения не совпадают");
            }
...
//                            std::string(property.c_str()) + "'.");
                    m_results->pushError(m_context, "Отсутствует обязательное поле '" +
                                         std::string(property.c_str()) + "'.");
                }
            ...
            for (const ValueType &dependencyName : dependencyNames) {
                ...
                if (m_object.find(dependencyNameKey) == m_object.end()) {
                    ...
                    if (m_results) {
                        ...
                        m_results->pushError(m_context, "Отсутствует зависимость '" + dependencyNameKey + "'.");
                    } else {
                        return false;
                    }
                }
...
//                    " against corresponding item schema.");
                m_results->pushError(newContext, "Элемент #" + std::to_string(index) +
                                     " не соответствует его описанию в схеме.");
            }
...
//                                patternPropertyStr + "'.");
                        m_results->pushError(m_context, "Провал валидации поля по шаблону '" +
                                             patternPropertyStr + "'.");
                    }
...
//                        propertyNameKey + "'.");
                m_results->pushError(m_context, "Провал валидации поля '" +
                        propertyNameKey + "'.");
            }
        ...
        bool operator()(const StringType &propertyName, const Subschema *schemaDependency) const
        {
            ...
            if (!m_validationVisitor.validateSchema(*schemaDependency)) {
                ...
                if (m_results) {
                    ...
                    m_results->pushError(m_context, "Провал валидации по зависимой схеме.");
                } else {
                    return false;
                }
            }
            ...
            if (m_results) {
                m_results->pushError(m_context,
                        //"Failed to validate against child schema #" + std::to_string(index) + ".");
                         "провал валидации по дочерней схеме #" + std::to_string(index) + ".");
            }
